...
## 21. Implementação da Fase 6.1B5 — RetrievalBundle combinado

### Consolidação por turno

`combined-evidence.ts` consolida, por `internalMessageId`, as evidências estruturadas já observadas pelo V1 e os fatos estruturados do turno/sessão. O bundle usa `RETRIEVAL_BUNDLE_VERSION=1` e separa evidências oficiais, documentos/RAG, memória, cliente, sessão, ferramentas, confirmação humana e contexto. O V2 não inicia buscas, não gera embeddings e não recebe conteúdo integral.

O cliente atual atravessa somente chaves estruturadas de fatos sobre o próprio cliente. A sessão atravessa fatos confirmados do `ConversationState` atual, sempre com `sourceMessageId`, timestamp e escopo de conversa/contextVersion; `pendingFields` não são convertidos em evidência. Ambos permanecem sem conteúdo no manifesto.

### Deduplicação e decisões

Cada evidência é deduplicada por `evidenceId`. Repetição com o mesmo hash mantém uma entrada; o mesmo ID com hash divergente é removido e registrado como inválido/conflitante. Escopo inválido, freshness expirada e proveniência insuficiente não participam da resolução. A decisão continua independente por categoria: score/confidence não resolvem conflito autoritativo, memória e sessão não autorizam política empresarial, RAG estático não autoriza disponibilidade/agendamento e ausência de fonte gera `SAFE_UNAVAILABLE` na própria categoria solicitada.

O manifesto registra decisões, vencedores/rejeitados, categorias autorizadas, contextuais e indisponíveis, conflitos, missing categories, status/duração dos adapters, contagens e flags de conteúdo persistido como `false`. `ResponsePlan.evidenceMetadata` é apenas metadata Shadow; não produz resposta nem altera o V1.

### Fail-safe e compatibilidade

Os adapters continuam read-only e recebem apenas snapshots já produzidos pelo V1. Falha de um adapter não impede os demais; falha total registra somente `EVIDENCE_PIPELINE_ERROR` sanitizado. O V1 mantém PromptCompiler, provider, ferramentas e outbound. O V2 permanece sem provider, ferramentas, memória/RAG de escrita e outbound.

### Limitações e plano da Fase 6.1C
