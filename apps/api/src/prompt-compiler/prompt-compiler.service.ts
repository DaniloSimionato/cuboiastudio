...
export class PromptCompilerService {
  compile(input: PromptCompilerInput): AiChatCompletionMessage[] {
    ...
    if (emojiUsage === "none")
      behaviorInstructions += `- Uso de emojis: PROIBIDO. Não use nenhum emoji.\n`;
    if (emojiUsage === "low")
      behaviorInstructions += `- Uso de emojis: Baixo. Use raramente, apenas para dar um tom leve.\n`;
    if (emojiUsage === "moderate")
      behaviorInstructions += `- Uso de emojis: Moderado. Pode usar para deixar a conversa amigável.\n`;
    ...
    if (officialBusinessContext) {
      messages.push({
        role: "system",
        content: officialBusinessContext.promptBlock,
      });
    }
    ...
    if (calendarContext) {
      ...
      if (calendarContext.serverTime) {
        calendarLines.push(
          `- Data e hora atual do servidor: ${calendarContext.serverTime}. Sempre utilize esta referência para interpretar expressões de data/hora relativas informadas pelo usuário.`,
        );
      }
